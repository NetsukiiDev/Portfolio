...
type Group = "personal" | "seo" | "home" | "sections" | "pages" | "maintenance";
...
const SECTIONS: { id: string; label: string; entries: Entry[] }[] = [
  ...
  },
  {
    ...
    label: "Sezioni",
    entries: [
      { group: "sections", field: "featuredProjects", label: "Progetti · titolo" },
      { group: "pages", field: "projects", label: "Progetti · riga", multiline: true },
      { group: "sections", field: "tools", label: "Stack · titolo" },
      { group: "sections", field: "skills", label: "Competenze · titolo" },
      { group: "pages", field: "skills", label: "Competenze · riga", multiline: true },
      { group: "sections", field: "experience", label: "Esperienza · titolo" },
      { group: "pages", field: "experience", label: "Esperienza · riga", multiline: true },
      { group: "sections", field: "about", label: "Chi sono · titolo" },
      { group: "sections", field: "recentPosts", label: "Blog · titolo" },
...
function readGroup(settings: Settings, group: Group): Bag {
  ...
  if (group === "sections") return settings.sections.translations as unknown as Bag;
...
export function LanguageForm({ settings }: { settings: Settings }) {
  ...
  async function saveAll() {
    ...
    try {
      ...
      // what gets stored.
      const groups: Record<Group, Bag> = { personal: {}, seo: {}, home: {}, sections: {}, pages: {}, maintenance: {} };
      for (const group of ["personal", "seo", "home", "sections", "pages", "maintenance"] as Group[]) {
      ...
      const res = await fetch("/api/settings", {
        ...
        body: JSON.stringify({
          ...
          sections: { ...settings.sections, translations: groups.sections },
